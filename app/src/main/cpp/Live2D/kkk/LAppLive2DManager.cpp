...
{
    //修改成固定，不用Demo的自动扫描所有模型，之后能通过changeScence（）调换模型
    ReleaseAllModel();
    _models.PushBack(new LAppModel());

    const csmString modelPath = "Zombie/";
    const csmString modelJson = "Zombie.model3.json";

    _models[0]->LoadAssets(modelPath.GetRawString(), modelJson.GetRawString());
...
//
//    ReleaseAllModel();
//    _models.PushBack(new LAppModel());//LAppModel()是创建一个模型
